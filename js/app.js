const app = {
  ...
  init() {
    ...
    this.domElems.stopButton.addEventListener('click', this.stopGameHandler.bind(this));
    this.domElems.changeBoardButton.addEventListener('click', this.changeBoardHandler.bind(this));
  ...
  drawBoard() {
    for (let rowIndex = 0; rowIndex < this.grid.rows; rowIndex++) {
      ...
      for (let cellIndex = 0; cellIndex < this.grid.cols; cellIndex++) {
        ...
        for (let trapIndex = 0; trapIndex < this.traps.length; trapIndex++) {
          if (this.traps[trapIndex].x === cellIndex && this.traps[trapIndex].y === rowIndex) {
    ...
    if (this.player.x === this.targetCell.x && this.player.y === this.targetCell.y) {
      const playerEl = this.createPlayer();
  ...
  moveForward() {
    ...
    if (this.player.direction === 'right' && this.player.x < this.grid.cols - 1) {
      this.player.x += 1;
      this.player.nbMoves += 1;
      ...
      this.player.y -= 1;
      this.player.nbMoves += 1;
    } else if (this.player.direction === 'down' && this.player.y < this.grid.rows - 1) {
      this.player.y += 1;
      this.player.nbMoves += 1;
    } else {
      this.collision = true;
    ...
    if (this.player.isDead) {
      ...
      setTimeout(() => {
        if (this.player.hearts <= 0) {
          ...
          this.domElems.replayButton.addEventListener('click', this.replayButtonHandler.bind(this));
          this.music.gameOver.play();
          return;
        }
  ...
  increaseDifficulty(level) {
    if (level % 2 === 0) {
      this.traps = this.addTraps(level * 2, 2, this.grid.cols, 2, this.grid.rows);
    }
  },
  ...
  checkPlayerPosition(minValue, maxValue) {
    ...
    for (let i = 0; i < this.traps.length; i++) {
      if (playerPosition === this.traps[i].x || playerPosition === this.traps[i].y) {
        return (playerPosition = 0);
      }
    }

    if (playerPosition >= maxValue || playerPosition < 0 || playerPosition === maxValue) {
      return (playerPosition = 0);
    } else {
  ...
  addTraps(nbTraps, minXValue, maxXValue, minYValue, maxYValue) {
    ...
    for (let i = 0; i < nbTraps; i++) {
      ...
      } while (trapItem.x === this.targetCell.x && trapItem.y === this.targetCell.y);
    }
    return trapsArray;
  },
  ...
  jump() {
    ...
    if (this.player.direction === 'right' && this.player.x < this.grid.cols - 2) {
      this.player.x += 2;
      this.player.nbMoves += 1;
      ...
      this.music.jump.play();
      this.updateJumpCounter(this.player.maxJumps);
    } else if (this.player.direction === 'down' && this.player.y < this.grid.rows - 2) {
      this.player.y += 2;
      this.player.nbMoves += 1;
      this.player.maxJumps -= 1;
      this.music.jump.play();
